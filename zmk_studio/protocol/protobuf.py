...
class Protobuf:
    ...
    def encode_sint32(field_number: int, value: int) -> bytes:
        ...
        return encode_sint32(field_number, value)

    @staticmethod
    def encode_string(field_number: int, value: str) -> bytes:
        """Encode a string field."""
        return encode_string(field_number, value)

    @staticmethod
    def encode_ld(field_number: int, data: bytes) -> bytes:
        """Encode a length-delimited field."""
        return encode_ld(field_number, data)
...
def encode_zigzag(value: int) -> bytes:
    """
    Encode sint32 using ZigZag encoding then varint.
...
def encode_int32(field_number: int, value: int) -> bytes:
    ...
    Example:
        >>> encode_int32(1, -1)
        b'\\x08\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\xff\\x01'
    """
    if value == 0:
        return b""
    ...
    if value >= 0:
        ...
        return tag + encode_varint(value & 0xFFFFFFFF) + b"\x00\x00\x00\x00\x00"
...
def decode_message(data: bytes) -> dict[int, list[tuple[int, Any]]]:
    ...
    Args:
        data: Encoded protobuf message bytes
